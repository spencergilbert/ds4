...
extern "C" int ds4_gpu_store_raw_kv_tensor(ds4_gpu_tensor *raw_cache, const ds4_gpu_tensor *kv, uint32_t raw_cap, uint32_t row, uint32_t head_dim) {
    ...
    store_raw_kv_batch_kernel<<<(head_dim + 255) / 256, 256, 0, g_compute_stream>>>((float *)raw_cache->ptr, (const float *)kv->ptr, raw_cap, row, 1, head_dim);
...
extern "C" int ds4_gpu_store_raw_kv_batch_tensor(ds4_gpu_tensor *raw_cache, const ds4_gpu_tensor *kv, uint32_t raw_cap, uint32_t pos0, uint32_t n_tokens, uint32_t head_dim) {
    ...
    store_raw_kv_batch_kernel<<<(n + 255) / 256, 256, 0, g_compute_stream>>>((float *)raw_cache->ptr, (const float *)kv->ptr, raw_cap, pos0, n_tokens, head_dim);
...
extern "C" int ds4_gpu_attention_decode_heads_tensor(
    ...
    if (cfg->oldhip_attention_decode) {
        ...
        attention_decode_mixed_one_fast_oldhip_kernel<<<(unsigned)n_head, 256, shmem, g_compute_stream>>>(
                (float *)heads->ptr,
                (const float *)q->ptr,
                (const float *)raw_kv->ptr,
    ...
    if (!cuda_attention_score_buffer_fits(n_comp)) {
        if (!use_mask && head_dim == 512u) {
            ...
            attention_decode_mixed_heads8_online_kernel<<<online_grid, 256, 0, g_compute_stream>>>((float *)heads->ptr,
                                                                              sinks,
                                                                              (const float *)q->ptr,
                                                                              (const float *)raw_kv->ptr,
    ...
    attention_decode_mixed_kernel<<<grid, 256, 0, g_compute_stream>>>((float *)heads->ptr,
                                                 sinks,
                                                 (const float *)q->ptr,
                                                 (const float *)raw_kv->ptr,
                                                 n_comp ? comp_kv->ptr : raw_kv->ptr,
...
extern "C" int ds4_gpu_attention_prefill_raw_heads_tensor(ds4_gpu_tensor *heads, const void *model_map, uint64_t model_size, uint64_t sinks_offset, const ds4_gpu_tensor *q, const ds4_gpu_tensor *raw_kv, uint32_t n_tokens, uint32_t window, uint32_t n_head, uint32_t head_dim) {
    ...
    if (n_tokens > 1 && head_dim == 512 &&
        ...
        attention_static_mixed_heads8_online_kernel<<<grid, 256, 0, g_compute_stream>>>((float *)heads->ptr,
                                                                   sinks,
                                                                   (const float *)q->ptr,
                                                                   (const float *)raw_kv->ptr,
                                                                   (const float *)raw_kv->ptr,
    ...
    if (g_cublas_ready && n_tokens > 1 && head_dim == 512) {
        ...
        cublasStatus_t st = cublasSgemmStridedBatched(cuda_cublas_handle(),
                                                      CUBLAS_OP_T,
                                                      CUBLAS_OP_N,
                                                      (int)n_keys,
                                                      (int)n_tokens,
        ...
        attention_prefill_raw_softmax_kernel<<<sgrid, 256, 0, g_compute_stream>>>(scores, sinks, n_tokens, window, n_keys);
        if (!cuda_ok(cudaGetLastError(), "attention raw softmax launch")) return 0;
        const float one = 1.0f;
        st = cublasSgemmStridedBatched(cuda_cublas_handle(),
                                       CUBLAS_OP_N,
                                       CUBLAS_OP_N,
                                       (int)head_dim,
                                       (int)n_tokens,
        ...
        attention_prefill_unpack_heads_kernel<<<(n + 255) / 256, 256, 0, g_compute_stream>>>((float *)heads->ptr,
                                                                        out_tmp,
                                                                        n_tokens,
                                                                        n_head,
                                                                        head_dim);
    ...
    dim3 grid(n_tokens, n_head, 1);
    attention_prefill_raw_kernel<<<grid, 128, 0, g_compute_stream>>>((float *)heads->ptr,
                                                sinks,
                                                (const float *)q->ptr,
                                                (const float *)raw_kv->ptr,
...
static int attention_decode_batch_launch(
    ...
    if (!use_comp_mask && n_tokens > 1 && head_dim == 512 &&
        ...
        dim3 grid(n_tokens, (n_head + 7u) / 8u, 1);
        attention_decode_mixed_heads8_online_kernel<<<grid, 256, 0, g_compute_stream>>>((float *)heads->ptr,
                                                                   sinks,
                                                                   (const float *)q->ptr,
                                                                   (const float *)raw_kv->ptr,
                                                                   n_comp ? comp_kv->ptr : (const float *)raw_kv->ptr,
...
extern "C" int ds4_gpu_attention_indexed_mixed_batch_heads_tensor(
    ...
    if (n_tokens == 1u && cfg->oldhip_attention_decode) {
        ...
        const size_t shmem = (size_t)(rows ? rows : 1u) * sizeof(float);
        attention_decode_indexed_mixed_one_fast_oldhip_kernel<<<(unsigned)n_head, 256, shmem, g_compute_stream>>>(
                (float *)heads->ptr,
                (const float *)q->ptr,
                (const float *)raw_kv->ptr,
                comp_kv->ptr,
    ...
    /* WMMA two-pass fast path: no topk sort needed (the two-pass softmax is
     * order-independent) and the scratch/score buffers are per-block shared. */
    if (n_tokens > 1u &&
        head_dim == 512u &&
        top_k <= 512u &&
        !g_quality_mode &&
        ...
        if (comp_kv_f16) {
            attention_indexed_mixed_heads16_wmma_kernel<1><<<grid, 256, 0, g_compute_stream>>>(
                    (float *)heads->ptr,
                    sinks,
                    (const float *)q->ptr,
                    (const float *)raw_kv->ptr,
            ...
            attention_indexed_mixed_heads16_wmma_kernel<0><<<grid, 256, 0, g_compute_stream>>>(
                    (float *)heads->ptr,
                    sinks,
                    (const float *)q->ptr,
                    (const float *)raw_kv->ptr,
    ...
    if (n_tokens > 1u && top_k == 512u) {
        ...
        if (!sorted) return 0;
        indexed_topk_sort_512_asc_kernel<<<n_tokens, 512, 0, g_compute_stream>>>(sorted, topk_ptr, n_tokens);
    ...
    if (n_tokens > 1 &&
        ...
        if (!g_quality_mode && n_head <= 64u) {
            ...
            attention_indexed_mixed_heads8_online_kernel<8, 32><<<grid, 1024, 0, g_compute_stream>>>((float *)heads->ptr,
                                                                                sinks,
                                                                                (const float *)q->ptr,
                                                                                (const float *)raw_kv->ptr,
                                                                                comp_kv->ptr,
...
#endif
        dim3 grid(n_tokens, (n_head + 15u) / 16u, 1);
        attention_indexed_mixed_heads8_online_kernel<8, 16><<<grid, 512, 0, g_compute_stream>>>((float *)heads->ptr,
                                                                           sinks,
                                                                           (const float *)q->ptr,
                                                                           (const float *)raw_kv->ptr,
                                                                           comp_kv->ptr,
    ...
    dim3 grid(n_tokens, n_head, 1);
    attention_indexed_mixed_kernel<<<grid, 256, 0, g_compute_stream>>>((float *)heads->ptr,
                                                  sinks,
                                                  (const float *)q->ptr,
                                                  (const float *)raw_kv->ptr,
                                                  comp_kv->ptr,
...
static int attention_prefill_mixed_cublas_tiled(
    ...
    float *out_tmp = (float *)((char *)tmp + out_offset);
    attention_prefill_pack_mixed_kv_kernel<<<(kv_count + 255) / 256, 256, 0, g_compute_stream>>>(
            kv,
            (const float *)raw_kv->ptr,
            n_comp ? comp_kv->ptr : (const float *)raw_kv->ptr,
            (int)comp_kv_f16,
    ...
    for (uint32_t t0 = 0; t0 < n_tokens; t0 += tile_tokens) {
        ...
        attention_prefill_mixed_softmax_tile_kernel<<<sgrid, 256, 0, g_compute_stream>>>(
                scores,
                sinks,
                use_comp_mask ? (const float *)comp_mask->ptr : NULL,
                use_comp_mask,
        ...
        attention_prefill_unpack_heads_kernel<<<(n + 255) / 256, 256, 0, g_compute_stream>>>(
                heads_tile,
                out_tmp,
                nt,
                n_head,
...
static int attention_prefill_mixed_launch(
    ...
    if (g_cublas_ready && n_tokens > 1 && head_dim == 512) {
        ...
        dim3 sgrid(n_tokens, n_head, 1);
        attention_prefill_mixed_softmax_kernel<<<sgrid, 256, 0, g_compute_stream>>>(
                scores,
                sinks,
                use_comp_mask ? (const float *)comp_mask->ptr : NULL,
                use_comp_mask,
    ...
    dim3 grid(n_tokens, n_head, 1);
    attention_prefill_mixed_kernel<<<grid, 256, 0, g_compute_stream>>>((float *)heads->ptr,
                                                  sinks,
                                                  (const float *)q->ptr,
                                                  (const float *)raw_kv->ptr,
                                                  n_comp ? comp_kv->ptr : (const float *)raw_kv->ptr,
...
extern "C" int ds4_gpu_attention_output_q8_batch_f16_tensor(
    ...
    attention_pack_group_heads_f16_kernel<<<(heads_h_count + 255u) / 256u, 256, 0, g_compute_stream>>>(
            heads_h,
...
extern "C" int ds4_gpu_attention_output_q8_batch_tensor(
    ...
    if (!attn_output_cublas) {
        if ((group_dim & 31u) == 0u && rank <= UINT32_MAX && n_tokens <= UINT32_MAX) {
            ...
            grouped_q8_0_a_f32_batch_warp8_kernel<<<grid_a, 256, 0, g_compute_stream>>>(
                    (float *)low->ptr,
                    out_a,
                    (const float *)heads->ptr,
                    group_dim,
    ...
    if (out_a_f16) {
        if (cuda_runtime_config()->attention_output_cublas_all &&
            ...
            if (out_b_f16 || out_b_f16_t) {
                ...
                __half *heads_h = (__half *)tmp;
                __half *low_h = (__half *)((char *)tmp + low_h_offset);
                attention_pack_group_heads_f16_kernel<<<(heads_h_count + 255) / 256, 256, 0, g_compute_stream>>>(
                        heads_h,
                        (const float *)heads->ptr,
                        n_tokens,
                        n_groups,
        ...
        attention_unpack_group_low_kernel<<<(low_tmp_count + 255) / 256, 256, 0, g_compute_stream>>>(
                (float *)low->ptr,
                low_packed,
                n_tokens,
                n_groups,
        ...
        quantize_q8_0_f32_kernel<<<qgrid, 32, 0, g_compute_stream>>>(xq,
                                                xscale,
                                                (const float *)heads->ptr,
                                                group_dim,
                                                blocks_a);
        ...
        dim3 grid_a(((unsigned)low_dim + 7u) / 8u, (unsigned)n_tokens, 1);
        grouped_q8_0_a_preq_warp8_kernel<<<grid_a, 256, 0, g_compute_stream>>>((float *)low->ptr,
                                                          out_a,
                                                          xq,
                                                          xscale,
                                                          group_dim,
...
extern "C" int ds4_gpu_attention_output_low_q8_tensor(
    ...
    if (!cuda_runtime_config()->disable_splitk_attn_out_low &&
        ...
        grouped_q8_0_a_partial16_w32_kernel<<<dim3((unsigned)((low_dim + 31u) / 32u), 8u),
                                              1024u, 512u * sizeof(float), g_compute_stream>>>(
                partial,
                out_a,
                (const float *)heads->ptr,
                n_groups,
        ...
        q8_partial_sum8_kernel<<<(unsigned)((low_dim + 255u) / 256u), 256, 0, g_compute_stream>>>(
                (float *)low->ptr,
                partial,
                (uint32_t)low_dim);
    ...
    if (!cuda_q8_prequant_decode_enabled()) {
        if ((group_dim & 31u) == 0u && group_dim <= 4096u &&
            ...
            grouped_q8_0_a_f32_sharedx_rows_w32_2row_kernel<<<
                    (unsigned)((low_dim + rows_per_block - 1u) /
                               rows_per_block),
                    1024u,
                    (size_t)group_dim * sizeof(float), g_compute_stream>>>(
                    (float *)low->ptr,
                    out_a,
                    (const float *)heads->ptr,
                    n_groups,
        ...
        grouped_q8_0_a_f32_warp8_kernel<<<
                ((unsigned)low_dim + 7u) / 8u, 256, 0, g_compute_stream>>>(
                (float *)low->ptr,
                out_a,
                (const float *)heads->ptr,
                group_dim,
