...
static int cuda_matmul_q8_0_tensor_labeled(ds4_gpu_tensor *out, const void *model_map, uint64_t model_size, uint64_t weight_offset, uint64_t in_dim, uint64_t out_dim, const ds4_gpu_tensor *x, uint64_t n_tok, const char *label) {
    ...
    if (n_tok > 1) {
        ...
        if ((in_dim & 31u) == 0u && out_dim <= UINT32_MAX && n_tok <= UINT32_MAX) {
            ...
            /* Small decode batches (M <= 16) are what the DSpark verify (M=5)
             * and draft (M=6) run; the fixed 32-token tile staged 32/M of the
             * work as wasted zero rows.  Shrink the token tile to the batch
             * size.  Bit-exact: each output element's sum order is unchanged,
             * only the per-block token grouping differs. */
            const uint32_t tile =
                n_tok <= 2u ? 2u :
                n_tok <= 4u ? 4u :
                n_tok <= 8u ? 8u :
