...
static uint64_t cuda_model_cache_limit_bytes(void) {
    ...
    (void)free_b;
    /* The span-staging arena needs enough headroom to hold the prefill
     * working set alongside the expert-cache ring and KV buffers.  The
     * prior 1/3-of-total cap was too tight for machines where the ring
     * alone already consumes half the working set.  Bump to 3/4 of total
     * while keeping the minimum safe floor. */
    uint64_t limit = (total_b / 4ull) * 3ull;
