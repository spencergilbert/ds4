...
extern "C" int ds4_gpu_compressor_store_batch_tensor(
    ...
    if (!ape) return 0;
    uint64_t n = (uint64_t)n_tokens * width;
    compressor_store_kernel<<<(n + 255) / 256, 256, 0, g_compute_stream>>>(
            (const float *)kv->ptr,
            (const float *)sc->ptr,
            (float *)state_kv->ptr,
...
extern "C" int ds4_gpu_compressor_update_tensor(
    ...
    compressor_update_pool_kernel<<<(head_dim + 255) / 256, 256, 0, g_compute_stream>>>(
    ...
    if (ok && ratio == 4u) {
        uint64_t half = 4ull * width;
        compressor_shift_ratio4_kernel<<<(half + 255) / 256, 256, 0, g_compute_stream>>>(
...
extern "C" int ds4_gpu_compressor_prefill_tensor(
    ...
    fill_f32_kernel<<<(state_n + 255) / 256, 256, 0, g_compute_stream>>>((float *)state_score->ptr, state_n, -INFINITY);
    ...
    if (ratio == 4u) {
        if (cutoff >= ratio) {
            ...
            compressor_set_rows_kernel<<<(n + 255) / 256, 256, 0, g_compute_stream>>>(
    ...
    if (n_comp != 0) {
        ...
        compressor_prefill_pool_kernel<<<grid, 256, 0, g_compute_stream>>>(
