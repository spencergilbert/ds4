...
- **Q8_0 LUT dequant (implemented 2026-08-14, neutral)**: a 256-entry
  int8->fp16 shared lookup replacing the per-element VALU I2F+F2F in the
  8-warp batch GEMM, bit-exact by construction (the LUT value is the exact
  (_Float16)(float)(int8_t) conversion). Measured neutral: 32K 261.5 vs
  262.0, 64K 245.8 vs 247.3 t/s. The dequant is not the bottleneck in the
  8-warp form (73f0a6a already hid it behind the other warps' MMA issue);
